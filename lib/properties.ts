...
import type { PricePeriod, RentalKind } from "./rental-kinds";
import {
  ALL_MOCK_PROPERTIES,
...
export interface Property {
  ...
  created_at: string;
  slug: string;
  /** How many covered parking spots — 0 for listings that have none */
  parking?: number | null;
  property_type?: string | null;

  /* ── Rental-only fields. All null on a sale, enforced by a CHECK in 0004 ── */
  rental_kind?: RentalKind | null;
  /** Which unit price_value is quoted in */
  price_period?: PricePeriod | null;
  min_stay_months?: number | null;
  /** Deposit expressed in months of rent, so 1.5 is a real value */
  ...
  /**
   * Monthly-comparable rent, generated by Postgres from price_value and
   * price_period. Read-only — never write this column.
   */
  rent_monthly_eq?: number | null;
}
...
/**
 * The operation the homepage lists. Rentals live on /rent, which keeps the two
 * price units apart: a filter written for 600,000 means nothing against 1,200
 * a month. Migration 0004 rewrote every 'buy' and NULL to 'sale', so this
 * equality does not silently drop rows.
 */
const SALE = "sale";
...
export async function getMarketProperties(
  ...
  // The mock generator also produces rentals; the homepage is sale-only now
